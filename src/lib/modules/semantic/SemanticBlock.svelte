<script lang="ts">
	...
	import { buildVariant, type SemanticData, type SemanticVariant } from './SemanticModule';
	...
	let default_from_preset = $derived(buildVariant(preset));
	...
	let baseColor = $derived(data.baseColor || default_from_preset.baseColor || '#FFFFFF');

	let border_color = $derived(shadeColor(baseColor, -20));
	let prefix_color = $derived(shadeColor(baseColor, 0));
	let prefix_text = $derived('white');
</script>

<div class="relative mx-20 my-2 flex flex-1 flex-col">
	{#if editable || spoilerMode || data.prefix?.trim().length !== 0 || data.title?.trim().length !== 0}
		<div
		...
		>
			{#if editable || data.prefix?.trim().length !== 0}
				...
				>
					...
					{:else if (preset.prefix?.type === 'static' || (preset.prefix?.type === 'editable' && !editable)) && data.prefix?.trim() !== ''}
						<div>
							{data.prefix}
						</div>
					{/if}
			...
			{#if preset.title?.type === 'editable' && editable}
				...
				></div>
			{:else if (preset.title?.type === 'static' || (preset.title?.type === 'editable' && !editable)) && data.title?.trim() !== ''}
